...
{
	/**
	 *   Creates a string with variable substitutions. Very similiar to sprintf, specially python's sprintf
	 ...
	 *   
	 *   sprintf("This is an %s lybrary for creating %s", "Actioscript 3.0", "strings");
	 *   // outputs: "This is an Actioscript 3.0 lybrary for creating strings";
	 *   // you can also format numbers:
	 *   
	 *   sprintf("You can also display numbers like PI: %f, and format them to a fixed precision, such as PI with 3 decimal places %.3f", Math.PI, Math.PI);
	 ...
	 *   
	 *   sprintf("My name is %(name)s and I am a %(ocupation)s. You can read more on my personal %(website)s, or reach me through my %(email)s", userInfo);
	 ...
	 *   sprintf("Today is %d/%m/%Y", date, date, date)
	 *   
	 *   </pre>
	 *   @see br.com.stimuli.string 
	 */
	public function sprintf(raw : String, ...rest) : String{
		/**
		 * Pretty ugly!
		 *   basicaly
		...
		while (Boolean(result)){
			...
			if (isPositionalSubts){
				...
				replacementValue = rest[numMatches];
			...
			if (replacementValue != undefined){
				...
				}else if(formater == PERCENT_FORMATER){
					match.replacement= PERCENT_FORMATER;
					numMatches--;
